...
attention:The input objects of 4DCNN are amino acids and residues at the interaction interface of RNA-protein within 6A, which can be obtained through ./6A_calculation and ./npy_preparation. 
...
For the case where both RNA and protein are single-stranded, we provide a simple script(test.sh) to use the flow from pdb files to scoring.
### For structures containing multiple chains, you need to manually delete protein-protein, RNA-RNA self-interactions from the output file in the first step.
